...
use onetagger_tagger::{LyricsLine, LyricsLinePart, Lyrics, Track, TrackNumber, AutotaggerSourceBuilder, PlatformInfo, TaggerConfig, AutotaggerSource, AudioFileInfo, MatchingUtils, PlatformCustomOptions, PlatformCustomOptionValue, supported_tags, TrackMatch, SupportedTag};
...
impl AppleMusic {
    ...
    pub fn fetch_token(&self) -> Result<(), Error> {
        // Fetch the token
        ...
        let index_js = re.captures(&body)?.ok_or(anyhow!("Unable to find index_js url"))?.get(1).ok_or(anyhow!("Unable to get index_js url"))?.as_str();
        let index_js = self.client.get(format!("https://music.apple.com/assets/index.{index_js}.js")).send()?.text()?;
        let re = Regex::new("(?=eyJh)(.*?)(?=\")").unwrap();
        let token = re.captures(&index_js)?.ok_or(anyhow!("Unable to find token"))?.get(1).ok_or(anyhow!("Unable to find token"))?.as_str();
        *self.access_token.lock().unwrap() = Some(token.to_string());
        ...
        if !r["meta"]["subscription"]["active"].as_bool().unwrap_or(false) {
            return Err(anyhow!("Not subscribed!"));
        }
        // Get storefront
        let storefront = r["meta"]["subscription"]["storefront"].as_str().ok_or(anyhow!("Unable to get storefront!"))?;
    ...
    /// Do a GET request
    fn get<O: DeserializeOwned>(&self, path: &str, query: &[(&str, &str)]) -> Result<O, Error> {
        // Get token
    ...
    /// Get the lyrics
    pub fn lyrics(&self, song_id: &str) -> Result<Lyrics, Error> {
        let lyrics: Value = self.get(&format!("songs/{song_id}/lyrics"), &[])?;
        let ttml = lyrics["data"][0]["attributes"]["ttml"].as_str().ok_or(anyhow!("Missing TTML"))?;
        Ok(Self::parse_ttml(ttml, &self.language)?)
    }

    /// Parse TTML from Apple Music
    fn parse_ttml(ttml: &str, language: &str) -> Result<Lyrics, Error> {
            ...
            match token {
                ...
                Token::Attribute { local, value, .. } => {
                    ...
                    if is_line_header {
                        ...
                        match local.as_str() {
                            "begin" => line.start = Some(Lyrics::parse_lrc_timestamp(&value)?.into()),
                            "end" => line.end = Some(Lyrics::parse_lrc_timestamp(&value)?.into()),
                            _ => {}
                        }
                    }
                    ...
                    if is_synced_line {
                        ...
                        match local.as_str() {
                            "begin" => part.start = Some(Lyrics::parse_lrc_timestamp(&value)?.into()),
                            "end" => part.end = Some(Lyrics::parse_lrc_timestamp(&value)?.into()),
                            _ => {}
                        }
                    }
                },
...
impl AutotaggerSource for AppleMusic {
    ...
    fn extend_track(&mut self, track: &mut Track, config: &TaggerConfig) -> Result<(), Error> {
        if config.tag_enabled(SupportedTag::SyncedLyrics) || config.tag_enabled(SupportedTag::UnsyncedLyrics) {
...
impl Into<Track> for ItemMeta<SongAttributes> {
    fn into(self) -> Track {
        ...
        Track {
            ...
            duration: Duration::from_millis(self.attributes.duration_in_millis).into(),
...
impl AutotaggerSourceBuilder for AppleMusicBuilder {
    ...
    fn get_source(&mut self, config: &TaggerConfig) -> Result<Box<dyn AutotaggerSource>, Error> {
        ...
        // Create new
        let amc: AppleMusicConfig = serde_json::from_value(config.custom.get("apple_music").ok_or(anyhow!("Missing custom config"))?.to_owned())?;
